...
def cameraOpen():
    ...
    with tf.Graph().as_default():
        with tf.Session() as sess:     
            ...
            # emb_dir=r'C:\XXXX_CodeRepository\GraduationDesign_exe\core_Rgnz\emb_img\usr_yueqingming'
            # all_obj=[]
            ...
            for emb_dir in os.listdir(emb_dirs):
                for i in os.listdir(emb_dirs+"\%s"%emb_dir):
                    all_obj.append(i)
                    img = misc.imread(os.path.join(emb_dirs+"\%s" %emb_dir, i), mode='RGB')
                    prewhitened = facenet.prewhiten(img)
                    image.append(prewhitened)
                    nrof_images = nrof_images + 1
