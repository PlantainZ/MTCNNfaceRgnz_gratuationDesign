...
def openCamera():
    ...
    def video_loop():
            ...
            if (mark):
                ...
                for rec_position in range(temp_num):
                    cv2.rectangle(frame, (bounding_box[rec_position, 0], bounding_box[rec_position, 1]),
                                  (bounding_box[rec_position, 2], bounding_box[rec_position, 3]), (0, 255, 0), 2, 8, 0)
        ...
        if success:
            ...
            cv2image = cv2.cvtColor(frame, cv2.COLOR_RGB2RGBA)#转换颜色从BGR到RGBA
    ...
    def clip_img(key,num,frame):
        if key == ord('p'):
            # 保存一张图像
            num = num + 1
            filename = "frames_%s.jpg" % num
            cv2.imwrite(filename, frame)
            print("WWWWWrite success!!")
    ...
    with tf.Graph().as_default():
        with tf.Session() as sess:
            ...
            # btn = Button(root, text="注册", command=take_snapshot)
            # btn.pack(fill="both", expand=True, padx=10, pady=10)
            # btn = Button(root, text="登陆", command=take_snapshot)
            # btn.pack(fill="both", expand=True, padx=10, pady=10)

            # user information
            Label(root, text='User name: ').place(x=220, y=500)
            Label(root, text='Password: ').place(x=220, y=525)

            var_usr_name = StringVar()
            var_usr_name.set('example@python.com')
            entry_usr_name = Entry(root, textvariable=var_usr_name)
            entry_usr_name.place(x=310, y=500)
            var_usr_pwd = StringVar()
            entry_usr_pwd = Entry(root, textvariable=var_usr_pwd, show='*')
            entry_usr_pwd.place(x=310, y=525)

            btn_login = Button(root, text='  登 录  ', command=take_snapshot)
            btn_login.place(x=250, y=570)
            btn_sign_up = Button(root, text='  注 册  ', command=take_snapshot)
            btn_sign_up.place(x=350, y=570)
