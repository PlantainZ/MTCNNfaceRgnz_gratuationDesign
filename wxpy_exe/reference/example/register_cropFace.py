...
class cropFace():
    ...
    def dection(self):
        ...
        # img_dir=r'C:\XXXX_CodeRepository\GraduationDesign_exe\core_Rgnz\train_dir\%s' %self.usr_acc
        img_dir=r'..\..\..\core_Rgnz\train_dir\%s' %self.usr_acc
        img_path_set=[]
        ...
        images = self.load_and_align_data(img_path_set, 160, 44, 1.0)


        # 改为emb_img文件夹的绝对路径
        # emb_dir=r'C:\XXXX_CodeRepository\GraduationDesign_exe\core_Rgnz\emb_img\%s' %self.usr_acc
        emb_dir=r'..\..\..\core_Rgnz\emb_img\%s' %self.usr_acc
        ...
        count=0
        for file in os.listdir(img_dir):
            misc.imsave(os.path.join(emb_dir,"emb_%s" %file),images[count])
            count=count+1
