...
class InquireOp(UserOperation):
    ...
    def newRgnzCmr(self,event):
        ...
        Publisher.subscribe(self.updateDisplay, "closeDia")
...
class AddOp(UserOperation):
    ...
    def rgstDisplay(self, msg):
        ...
        if(tips):
            print("注册完成！"+tips)
            self.stu_gender.SetLabel('')
            self.stu_age.SetLabel('')
            self.stu_cid.SetLabel('')
            self.stu_classid.SetLabel('')
        # if isinstance(t, str):
        #     self.displayLbl.SetLabel("%s" % t)
        # else:
        #     self.displayLbl.SetLabel("多多多你是个大佬呀！！" )
...
class detailImg(DelOp):
    ...
    def __init__(self,usr_acc,usr_name):
        ...
        Publisher.subscribe(self.imgListRefresh, "rgst")
